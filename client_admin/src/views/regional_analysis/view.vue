	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="电影名称" prop="film_title">
							  ...
							  v-if="user_group === '管理员' || (form['regional_analysis_id'] && $check_field('set','film_title')) || (!form['regional_analysis_id'] && $check_field('add','film_title'))" :disabled="disabledObj['film_title_isDisabled']"></el-input>
				...
				<el-form-item label="电影年份" prop="film_year">
							  ...
							  v-if="user_group === '管理员' || (form['regional_analysis_id'] && $check_field('set','film_year')) || (!form['regional_analysis_id'] && $check_field('add','film_year'))" :disabled="disabledObj['film_year_isDisabled']"></el-input>
				...
				<el-form-item label="电影评分" prop="film_rating">
							  ...
							  v-if="user_group === '管理员' || (form['regional_analysis_id'] && $check_field('set','film_rating')) || (!form['regional_analysis_id'] && $check_field('add','film_rating'))" :disabled="disabledObj['film_rating_isDisabled']"></el-input>
				...
				<el-form-item label="所属国家" prop="country">
							  ...
							  v-if="user_group === '管理员' || (form['regional_analysis_id'] && $check_field('set','country')) || (!form['regional_analysis_id'] && $check_field('add','country'))" :disabled="disabledObj['country_isDisabled']"></el-input>
				...
				<el-form-item label="电影类型" prop="film_type">
							  ...
							  v-if="user_group === '管理员' || (form['regional_analysis_id'] && $check_field('set','film_type')) || (!form['regional_analysis_id'] && $check_field('add','film_type'))" :disabled="disabledObj['film_type_isDisabled']"></el-input>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "regional_analysis_id",
				url_add: "~/api/regional_analysis/add?",
				url_set: "~/api/regional_analysis/set?",
				url_get_obj: "~/api/regional_analysis/get_obj?",
				url_upload: "~/api/regional_analysis/upload?",

				query: {
					"regional_analysis_id": 0,
				},
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/regional_analysis/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/regional_analysis/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/regional_analysis/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/regional_analysis/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/regional_analysis/view','get');
