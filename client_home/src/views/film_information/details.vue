<template>
	<div class="diy_details page_film_information" id="film_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_film_information'>
							<div_film_information :obj="obj"></div_film_information>
...
<script>
	import div_film_information from "@/components/diy/div_film_information.vue";
	...
	export default {
		...
		components:{
			...
			bar_title,
			form_editor,
			div_film_information
		},
		data(){
			return{
				url_get_obj: "~/api/film_information/get_obj?",
				...
				form_comment: {
					...
					source_field: "film_information_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/film_information/set?film_information_id=' + obj["film_information_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "film_information",
							source_field: "film_information_id",
							source_id: this.obj.film_information_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.film_information_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "film_information",
					source_field: "film_information_id",
					source_id: obj["film_information_id"],
					user_id
				};
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.film_title
																																												query.img = this.obj.movie_pictures
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/film_information/set?film_information_id=' + _this.obj["film_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
