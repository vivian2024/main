	...
	<div class="page_root" id="root_demo">
		...
		<div class="warp">
		    <div class="container">
				...
				<!-- 推荐电影信息模块(开始) -->
		<div class="warp" v-if="$check_action('/film_information/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/film_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_film_information = !show_list_film_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_film_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 电影信息推荐列表组件 -->
		                <list_film_information v-if="show_list_film_information" :list="list_film_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_film_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_film_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_film_information" :key="i" @click="to_details('film_information',o,'film_information_id')">
		                            <td v-for="(oj,n) in fields_film_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	import mixin from "@/mixins/page.js";
			import list_film_information from "@/components/diy/list_film_information.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
						show_list_film_information: true,
				list_film_information: [],
				vm_arr_film_information: [
					"film_information_id",
							"film_title",
							"film_year",
							"film_rating",
		...
		methods: {
			...
			get_film_information() {
				let url = "~/api/film_information/get_list?";
								// url = "~/api/film_information/get_list?orderby=hits desc";
								this.$get(url, {
									"orderby": "hits desc",
							"page": 1,
					"size": 8
					...
					if (json.result) {
						this.list_film_information = json.result.list;
					}
				})
			},
		...
		computed: {
				    list_table_regional_analysis() {
		        var list = this.list_film_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_film_information.map((o) => {
		                // 第二个中括号是对象的属性
